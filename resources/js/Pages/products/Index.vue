<template>
    ...
    <!-- <nav class="bg-white shadow mb-6">
      <div class="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
        <h1 class="text-xl font-bold text-blue-600">Clothing Store</h1>

        <div class="space-x-4">
          <Link href="/cart" class="text-gray-700 hover:text-blue-600">Cart</Link>
          <Link v-if="!auth.user" href="/login" class="text-gray-700 hover:text-blue-600">Login</Link>
          <Link v-if="!auth.user" href="/register" class="text-gray-700 hover:text-blue-600">Register</Link>
          <Link v-if="auth.user" href="/dashboard" class="text-gray-700 hover:text-blue-600">Dashboard</Link>
        </div>
      </div>
    </nav> -->

  <div class="p-4 sm:p-6 max-w-7xl mx-auto">
    <h1 class="text-2xl sm:text-3xl font-bold mb-6 text-gray-800">Our Products</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      >
        <div class="relative overflow-hidden rounded-lg mb-4 h-48 bg-gray-100">
          <img
            :src="product.first_image?.image_path || 'https://via.placeholder.com/300x300?text=No+Image'"
            alt="product.name"
            class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
          />
          <div class="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
        </div>
        <h2 class="text-lg font-bold text-gray-800 group-hover:text-blue-600 transition-colors">{{ product.name }}</h2>
